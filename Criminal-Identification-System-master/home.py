...
def basicPageSetup(pageNo):
    ...
    back_img = tk.PhotoImage(file="icons\\back.png")
...
def selectMultiImage(opt_menu, menu_var):
    ...
    if(len(path_list) < 5):
        ...
        back_img = tk.PhotoImage(file="icons\\previous.png")
        next_img = tk.PhotoImage(file="icons\\next.png")
...
def getEntries(entries,required):
    ...
    for i, entry in enumerate(entries):
        ...
        if (len(val) == 0 and required[i] == 1):
            ...
            return
        if (len(val) >40 and i == 0):
            messagebox.showerror("Name Length should not be grater than 10")
            return
        if (len(val) >40 and i == 1):
            messagebox.showerror("Father Name Length should not be grater than 10")
            return
        if (len(val) >40 and i == 2):
            messagebox.showerror("Mother Name Length should not be grater than 10")
            return
        if (len(val) >10 and i == 4):
            messagebox.showinfo("Valid DOB")
            return
        if (len(val) >10 and i == 5):
            messagebox.showinfo("Blood group Length should not be grater than 10")
            return
        if (len(val) >40 and i == 6):
            messagebox.showinfo("identification mark Length should not be grater than 10")
            return
        else:
...
######################################## Home Page ####################################
tk.Label(pages[0], text="Criminal Face Detection and Identification", fg="white", bg="#202d42",
      font="Arial 35 bold", pady=20).pack()

logo = tk.PhotoImage(file = "icons\\loogo.png")
